      ...
      <div className={styles.grid2} style={{ marginTop: "var(--space-lg)" }}>
        {assets.map((asset) => (
          <div key={asset.id} className={styles.card}>
            <p className={styles.muted}>{a.folders[asset.folder] || asset.folder}</p>
            <a href={asset.publicUrl} target="_blank" rel="noreferrer">
              {asset.r2Key}
            </a>
            {asset.altText ? <p>{pickLocale(asset.altText, "vi")}</p> : null}
            <p className={styles.muted} style={{ fontSize: "0.75rem" }}>
              ID: {asset.id}
            </p>
            <DeleteButton action={`/api/admin/media/${asset.id}`} confirmMessage="Xóa tệp media này?" />
          </div>
        ))}
      </div>
    </>
