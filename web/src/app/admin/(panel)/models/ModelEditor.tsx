    ...
    run(
      ...
      (saved) => {
        if (!model && saved && typeof saved === "object" && "id" in saved) {
          router.push(`/admin/models/${String(saved.id)}`);
        }
      },
    );
      ...
      {tab === "publish" && model ? (
        <section className={styles.section}>
          ...
          <div className={styles.actions}>
            ...
            <ConfirmDelete message={t("models.confirmDelete", { name: model.name.vi })} onConfirm={() => run(() => deleteModelAction(model.id), t("models.deleted"), () => router.push("/admin/models"))} />
          </div>
        </section>
      ) : null}
    </div>
